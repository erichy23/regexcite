...
## Usage
### my_add
A function designed for adding two numeric values.
Use the `my_add()` when the inputs are numeric values.
It will return error message if one or both of them are not.
...
The default value of the second parameter is 10, so if there's only one number given, it will add 10 to it and return the result.
```{r}
library(regexcite)
...
If one or both of the input parameters are missing, it will return NA as below.
```{r}
library(regexcite)
